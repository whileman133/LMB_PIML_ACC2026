...
if __name__ == "__main__":
    ...
    predict_thetass = adapter_thetass.wrap(normalizer_thetass.wrap(sr_thetass.predict))
    predict_phie = adapter_phie.wrap(normalizer_phie.wrap(sr_phie.predict))
    predict_if = adapter_if.wrap(normalizer_if.wrap(sr_if.predict))
    predict = lambda spme_state: {
    ...
    for dataset_key, dataset in datasets.items():  # train, test
        ...
        for series_key, series in dataset.items():  # cc, gitt, drive
            for sim_data in series:
                ...
                # Make plot directory.
                plot_dir = os.path.join('plots_sr', 'trained', dataset_key, series_key, spec_string)
                if not os.path.isdir(plot_dir):
                    os.makedirs(plot_dir)

                # Restrict plotted time domain for drive-cycle plots for improved readability.
                if series_key == 'drive':
                    time_ind = np.logical_and(3200 <= time, time <= 3500)
                else:
                ...
                plt.plot(time[time_ind], thetass2_hybrid[time_ind], '--', label=f'SR', zorder=10)
                ...
                plt.title(r'$\theta_\mathrm{ss}(\tilde{x}=2)$: ' f"{dataset_key.capitalize()} {spec_string}"
                          f" {series_key.upper()}")
                plt.legend(ncol=2).set_zorder(100)
                ...
                plt.plot(time[time_ind], 1000 * phie2_hybrid[time_ind], '--', label=f'SR', zorder=10)
                ...
                plt.plot(time[time_ind], if2_hybrid[time_ind], '--', label=f'SR', zorder=10)
                ...
                plt.plot(time[time_ind] / 60, vcell_hybrid[time_ind], '--', label=f'SR', zorder=10)
        ...
        data_dir = os.path.join('performance_data_sr', 'trained', dataset_key)
